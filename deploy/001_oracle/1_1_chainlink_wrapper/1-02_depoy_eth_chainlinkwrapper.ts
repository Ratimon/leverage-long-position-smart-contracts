...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
    ...
    const mockOracle = await getOrNull('OracleChainlinkMock_USD_per_ETH');


    if (mockOracle){
        OracleArgs[`chainlink`] = mockOracle.address;
    } else{
        const chainlinkAddress = ethChainlinkOracle
        OracleArgs[`chainlink`] = chainlinkAddress;
    }



    const deploymentName = "OracleChainlinkWrapper_USD_per_ETH"
...
export default func;
func.tags = ["1-1-02", "1-1" ,"eth-wrapper","oracle"];
func.dependencies = ["1-1-01"]